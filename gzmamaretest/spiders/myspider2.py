...
class Myspider2(scrapy.Spider):
    name = 'Myspider2'
    start_urls = ['http://www.gzmama.com',]
    # bashurl = '.html'
    dir_path = 'G:\gzMaMa'# 图片存放的路径
    urlformat='http://www.gzmama.com/'
    lastpage=0
    urlRun = start_urls
    ...
    download_delay = 1
    '''
    #rule1 匹配 正常的形如http://www.gzmama.com/forum-1881-1.html
    rule2 匹配 形如forum-1881-1.html 特殊帖子
    '''
    #Rule(LinkExtractor(allow=(r"http://www.gzmama.com/(?i)[0-9A-Z-]*.html$")),callback='_url_parse',follow=True),
    rules = [
    Rule(LinkExtractor(allow=(r"^[0-9A-Za-z-]*.*html$")),callback='parse_current',follow=True),
    ]
